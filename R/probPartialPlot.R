#' @title Random Forest probability scaled partial plots
#' @description Produces partial plots with probability distribution based on scaled margin distances.
#'       
#' @param x Object of class randomForest 
#' @param pred.data Training data.frame used for contructing the plot, 
...
#'           probPartialPlot(iris.rf, iris, i, "setosa", smooth=TRUE, raw=TRUE, rug=FALSE)
#'          }
probPartialPlot <- function(x, pred.data, xname, which.class, w, prob=TRUE, plot=TRUE,
                           smooth=FALSE, raw=FALSE, rug=FALSE, n.pt, xlab, ylab, main, ...) {  
    if (is.null(x$forest))
      stop("Object does not contain an ensemble \n")
	if(!x$type != "regression")	
      stop("Regression not supported \n")	   
            ...
            x.data[, xname] <- rep(x.pt[i], n)
              ...
              y.pt[i] <- weighted.mean(log(ifelse(pr[, focus] == 0,
                .Machine$double.eps, pr[, focus]))
                 - rowMeans(log(ifelse(pr == 0, .Machine$double.eps, pr))), w, na.rm=TRUE)
        }
    if( prob == TRUE) { y.pt <- scale.dist(y.pt) }       
	if (plot) {
      if(missing(xlab)) xlab=xname
		...
		if(smooth)  
		  ...
		  plot(x.pt, y.pt, type = "l", xlab=xlab, 
			   ylab=ylab, main=main, ...)		
      }	
    if (rug) {
      if (n.pt > 10) {
